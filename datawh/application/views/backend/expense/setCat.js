...
setCat.RequestCatList = function() {
    $.ajax({
        ...
        success: function(json) {
            if(json.WebService.ProductCatList && json.WebService.ProductCatList.ProductCat.length === 1){
                if(json.WebService.ProductCatList.ProductCat[0].ProductCatList){
                	setCat.catList = json.WebService.ProductCatList.ProductCat[0].ProductCatList.ProductCat;
                }
                else{
                    setCat.catList = json.WebService.ProductCatList.ProductCat;
                    setCat.catId = 0;
                }
                $("#setCat-header").text(json.WebService.ProductCatList.ProductCat[0].ProductCatName);
            }
        },
...
setCat.SetRequestUrl = function() {
	setCat.requestCatUrl = "ws/GetProductCatList.php";
	setCat.requestSetUrl = "ws/SetProductCat.php";
};
...
setCat.DrawNestable = function(catList) {
    ...
    $.each(catList, function(index,cat) {
        var name = cat.ProductCatName;
        var id = cat.ProductCatNodeId;
        var pId = cat.ParentCatId;
        var child = "";
        if(cat.ProductCatList){
            child = setCat.DrawNestable(cat.ProductCatList.ProductCat);
        }
