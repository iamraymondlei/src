...
function PolygonDrawingTool() {
  ...
  this._canvasScale = "1:1";
  this._customData = null;

  // this is the Shape that is shown during a drawing operation
  this._temporaryShape = go.GraphObject.make(go.Shape, { name: "SHAPE", fill: "lightgray", strokeWidth: 5 });
...
PolygonDrawingTool.prototype.canStart = function() {
  if(this.diagram.allowReshape) return false;
...
PolygonDrawingTool.prototype.doMouseDown = function() {
    if(this.diagram.allowReshape) return false;
  //this.diagram.isReadOnly = true; //if reshaping shape
  if (!this.isActive) {
    this.doActivate();
  }
  ...
  if(this.coordinateData.length > 1) this.mouseMoveCB(this.coordinateData,this.canvasScale,this.customData);
};
...
* @this {PolygonDrawingTool}
*/
PolygonDrawingTool.prototype.updateCoordinateData = function() {
    var geo = this.temporaryShape.geometryString;
    var geoAry = geo.replace(/F /,"").replace(/z/,"").split(" ");
    this.coordinateData = [];
    if(geoAry.length > 1){
        for(var i=0; i<geoAry.length; i++){
            var coordinateLength = this.coordinateData.length;
            var x = geoAry[i].replace(/M|L/,"");
            var y = geoAry[i+1].replace(/M|L/,"");
            
            if(coordinateLength < 1){
                this.coordinateData.push({"X":x,"Y":y});
            }
            else if( (this.coordinateData)[coordinateLength-1].X !== x || (this.coordinateData)[coordinateLength-1].Y !== y) {
                this.coordinateData.push({"X":x,"Y":y});
            }
            i++;
        }
...
PolygonDrawingTool.prototype.doMouseMove = function() {
  if (this.isActive) {
    //var point = this.diagram.lastInput.documentPoint;
    this.moveLastPoint(this.diagram.lastInput.documentPoint);
    this.updateCoordinateData();
    this.mouseMoveCB(this.coordinateData,this.canvasScale,this.customData);
  }
};
...
/**
* @name PolygonDrawingTool#coordinateData
* @function.
* @return {Object}
*/
Object.defineProperty(PolygonDrawingTool.prototype, "coordinateData", {
  get: function() { return this._coordinateData; },
  set: function(val) { this._coordinateData = val; }
});

/**
* @name PolygonDrawingTool#mouseMoveCB
* @function.
* @return {Object}
*/
Object.defineProperty(PolygonDrawingTool.prototype, "mouseMoveCB", {
  get: function() { return this._mouseMoveCB; },
  set: function(val) { this._mouseMoveCB = val; }
});

/**
* @name PolygonDrawingTool#canvasScale
* @function.
* @return {Object}
*/
Object.defineProperty(PolygonDrawingTool.prototype, "canvasScale", {
  get: function() { return this._canvasScale; },
  set: function(val) { this._canvasScale = val; }
});

/**
* @name PolygonDrawingTool#customData
* @function.
* @return {Object}
*/
Object.defineProperty(PolygonDrawingTool.prototype, "customData", {
  get: function() { return this._customData; },
  set: function(val) { this._customData = val; }
});
