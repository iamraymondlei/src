...
/**
 * Description of GetFileCatList
 *
 * @author icm
 */

class GetFileCatList extends WebService implements iWebService {
    ...
    function __construct() {
        ...
        if( $this->isPassed ){
            ...
            self::getFileCat();
            if($this->isDebug){ echo DatetimeUtil::getTime("getFileCat"); }
            self::closeDB();
        }
    ...
    private function getFileCat() {
        $params = $this->params;
        $tableName = 'FileCatNode';
        ...
        if($queryResult["state"] === 200){
            if(count($queryResult["body"])>0){
                if(isset($params["catId"])){
                    $catList = SortCatNode::RearrangeAry($queryResult["body"],"FileCatNodeId","ParentCatId","FileCatList","FileCat",$params["catId"]);
                }
                else{
                    $catList = SortCatNode::RearrangeAry($queryResult["body"],"FileCatNodeId","ParentCatId","FileCatList","FileCat");
                }
                $this->outputData = array("FileCatList"=>array("FileCat"=>$catList));
            }
        }
        else{
