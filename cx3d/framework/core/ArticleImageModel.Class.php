...
class ArticleImageModel extends Model
{
    ...
    {
        $articleIds = isset($fieldList["articleIds"])?$fieldList["articleIds"]:"";
        ...
        $sql_sel = "SELECT ai.* ";
        $sql_from = "FROM ArticleImage ai ";
        $sql_join = "";
        $sql_where = "WHERE 1=1 ";
        $sql_orderBy = "ORDER ai.`".$order."` ".$sort." ";
        $sql_limit = "LIMIT ".($pIndex-1) * $pSize.",".$pSize." ";

        if(isset($fieldList["articleIds"]) && strlen($articleIds)>0){
            $sql_where.= "AND ai.`ArticleListId` in ('$articleIds') ";
        }
    ...
    public function getCount($fieldList=array())
    {
        ...
        $result = $this->db->getCount("ArticleImage","ArticleImageId",$sql_where);
        return $result;
    }
    ...
    {
        $where = "ArticleImageId=".$id;
        $result = $this->db->update("ArticleImage",$data,$where);
        return $result;
    }
}